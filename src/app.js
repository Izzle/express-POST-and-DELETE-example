...
const users = [  // rather than storing our data in a database, we're storing it in memory for this example
  {
...
app.post('/user', (req, res, next) => { // eslint-disable-line no-unused-vars
  ...
  // All are required, check fi they were sent
  if (!username) {
    return res
      .status(400)
      .send('Username requried');
  }
  ...
  // make sure the username is correctly formatted (length)
  if (username.length < 6 || username.length > 20) {
    return res
      .status(400)
      .send('Username msut be between 6 and 20 characters');
  }
